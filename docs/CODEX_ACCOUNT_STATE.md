...
- The live master is stored as `openai_codex_ticket_v2_enabled`. During a rolling upgrade, the old `openai_codex_ticket_enabled` / `openai_codex_ticket_332_enabled` pair is read only as a fallback; the settings API never rewrites those rollback keys. If both legacy values are true, the fallback is fail-closed. The old global 332 value never selects a ticket length in v2.
...
Before routing traffic to a newly upgraded instance, inventory the accounts that were protected by the old global modes and explicitly configure each account through the admin API or UI. Map the old 292 population to `ticket_plan=pro` and the old 332 population to `ticket_plan=team`; when an account previously used both Astra and Sol, choose one model per account or create a deliberate second account. Do not infer a plan from subscription metadata, and do not enable newly imported or copied accounts without an operator decision. Keep the old pair and v1 ticket fields until the rollback window has closed.

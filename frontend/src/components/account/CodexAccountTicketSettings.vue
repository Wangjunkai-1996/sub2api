<template>
  <section class="space-y-3 rounded-lg border border-gray-200 p-4 dark:border-dark-600" data-testid="codex-account-ticket-settings">
    ...
    <template v-if="status">
      ...
      <div>
        <label :for="`codex-account-ticket-model-${accountId}`" class="input-label">{{ t('admin.accounts.stateTicket.modelLabel') }}</label>
        <select :id="`codex-account-ticket-model-${accountId}`" v-model="model" class="input w-full text-sm" :disabled="busy" data-testid="codex-account-ticket-model">
          <option value="gpt-6-astra">gpt-6-astra</option>
          <option value="gpt-5.6-sol">gpt-5.6-sol</option>
        </select>
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.stateTicket.modelHint') }}</p>
      </div>
      ...
      <p v-if="!status.fixed_proxy_configured" class="text-xs text-amber-700 dark:text-amber-300" data-testid="codex-account-ticket-egress-missing">{{ t('admin.accounts.stateTicket.businessEgressMissing') }}</p>
      ...
      <div class="flex flex-wrap gap-2">
        <button type="button" class="btn btn-primary btn-sm" :disabled="busy || !dirty || proxyChanged || (enabled && (!status.proxy_configured || !status.fixed_proxy_configured))" data-testid="codex-account-ticket-save" @click="save">
          {{ t('admin.accounts.stateTicket.save') }}
        </button>
        <button type="button" class="btn btn-secondary btn-sm" :disabled="busy || dirty || proxyChanged || !status.global_enabled || !status.enabled || !status.proxy_configured || !status.fixed_proxy_configured || status.state === 'harvesting'" data-testid="codex-account-ticket-harvest" @click="harvest">
...
async function save() {
  if (!status.value || busy.value || props.proxyChanged || (enabled.value && (!status.value.proxy_configured || !status.value.fixed_proxy_configured))) return
  busy.value = true
  saved.value = false
  error.value = ''
  revision++
...
async function harvest() {
  if (busy.value || dirty.value || props.proxyChanged || !status.value?.global_enabled || !status.value.enabled || !status.value.proxy_configured || !status.value.fixed_proxy_configured || status.value.state === 'harvesting') return
  busy.value = true
  saved.value = false
  error.value = ''
  revision++

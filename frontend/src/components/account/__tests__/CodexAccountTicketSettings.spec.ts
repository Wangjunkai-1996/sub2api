...
describe('CodexAccountTicketSettings', () => {
  ...
  it('opts in an account using the global pool without account proxy controls', async () => {
    ...
    await flushPromises()
    expect(api.saveCodexAccountTicket).toHaveBeenCalledWith(4, { enabled: true, ticket_plan: 'pro', model: 'gpt-6-astra' })
  ...
  it('requires verified business egress before enabling or harvesting but permits disabling', async () => {
    api.getCodexAccountTicket.mockResolvedValue(makeStatus({ proxy_configured: true, fixed_proxy_configured: false }))
    const wrapper = mountCard()
    await flushPromises()
    expect(wrapper.get(selector('egress-missing')).text()).toContain('businessEgressMissing')
    await wrapper.get(selector('enabled')).trigger('click')
    expect(wrapper.get(selector('save')).attributes('disabled')).toBeDefined()
    expect(api.saveCodexAccountTicket).not.toHaveBeenCalled()

    api.getCodexAccountTicket.mockResolvedValue(makeStatus({ enabled: true, proxy_configured: true, fixed_proxy_configured: false, state: 'error' }))
    await wrapper.setProps({ accountId: 5 })
    await flushPromises()
    expect(wrapper.get(selector('harvest')).attributes('disabled')).toBeDefined()
    await wrapper.get(selector('enabled')).trigger('click')
    expect(wrapper.get(selector('save')).attributes('disabled')).toBeUndefined()
    api.saveCodexAccountTicket.mockResolvedValue(makeStatus({ proxy_configured: true, fixed_proxy_configured: false }))
    await wrapper.get(selector('save')).trigger('click')
    await flushPromises()
    expect(api.saveCodexAccountTicket).toHaveBeenCalledWith(5, { enabled: false, ticket_plan: 'pro', model: 'gpt-6-astra' })
  })
  ...
  it('polls status without replacing edits, stops when hidden and reloads on reopen', async () => {
    ...
    await wrapper.get(selector('model')).setValue('gpt-5.6-sol')
    ...
    expect((wrapper.get(selector('model')).element as HTMLSelectElement).value).toBe('gpt-5.6-sol')
    expect(wrapper.find(selector('global-off')).exists()).toBe(true)
    ...
    expect(wrapper.get(selector('enabled')).attributes('aria-checked')).toBe('false')
    expect((wrapper.get(selector('plan')).element as HTMLSelectElement).value).toBe('pro')
    expect((wrapper.get(selector('model')).element as HTMLSelectElement).value).toBe('gpt-6-astra')
  })
  ...
  it('requires saving a manual Team selection and sends it without changing the proxy', async () => {
    ...
    await wrapper.get(selector('save')).trigger('click')
    await flushPromises()
    expect(api.saveCodexAccountTicket).toHaveBeenCalledWith(4, { enabled: true, ticket_plan: 'team', model: 'gpt-6-astra' })
    expect((wrapper.get(selector('plan')).element as HTMLSelectElement).value).toBe('team')
    expect(wrapper.get(selector('save')).attributes('disabled')).toBeDefined()
    expect(wrapper.get(selector('status')).text()).toContain('waiting')
  })

  it('saves an explicit target model and blocks harvest until the selection is saved', async () => {
    api.getCodexAccountTicket.mockResolvedValue(makeStatus({ enabled: true, proxy_configured: true, state: 'ready' }))
    const wrapper = mountCard()
    await flushPromises()
    expect((wrapper.get(selector('model')).element as HTMLSelectElement).value).toBe('gpt-6-astra')
    await wrapper.get(selector('model')).setValue('gpt-5.6-sol')
    ...
    expect(wrapper.get(selector('harvest')).attributes('disabled')).toBeDefined()
    expect(api.harvestCodexAccountTicket).not.toHaveBeenCalled()
    api.saveCodexAccountTicket.mockResolvedValue(makeStatus({ enabled: true, proxy_configured: true, model: 'gpt-5.6-sol', state: 'waiting' }))
    await wrapper.get(selector('save')).trigger('click')
    await flushPromises()
    expect(api.saveCodexAccountTicket).toHaveBeenCalledWith(4, { enabled: true, ticket_plan: 'pro', model: 'gpt-5.6-sol' })
    expect((wrapper.get(selector('model')).element as HTMLSelectElement).value).toBe('gpt-5.6-sol')
    expect(wrapper.get(selector('save')).attributes('disabled')).toBeDefined()
    expect(wrapper.get(selector('harvest')).attributes('disabled')).toBeUndefined()

    api.getCodexAccountTicket.mockResolvedValue(makeStatus({ enabled: true, proxy_configured: true, model: 'gpt-5.6-sol' }))

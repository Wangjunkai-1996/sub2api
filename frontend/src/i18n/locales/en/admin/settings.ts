export default {
    settings: {
      ...
      features: {
        ...
        cyberAccountCooldown: {
          title: 'OpenAI Cyber account cooldown',
          description: 'Cools down only the account that receives an explicit upstream cyber_policy response, without making an additional moderation request.',
          enabled: 'Enable stepped account cooldown',
          enabledHint: 'Applies only to OpenAI OAuth Pro accounts in the selected groups. Plus, API Key, and other-platform accounts are not cooled down.',
          windowSeconds: 'Quiet reset window (seconds)',
          firstSeconds: 'First cooldown (seconds)',
          escalatedSeconds: 'Repeated cooldown (seconds)',
          groupScope: 'Eligible account groups',
          groupScopeHint: 'The default group is 12. An account must still match OpenAI, OAuth, and plan_type=pro to be eligible.',
          searchGroups: 'Search group name, platform, type, or ID',
          disabledGroup: 'Disabled',
          noGroups: 'No matching groups',
          groupsRequired: 'Select at least one Cyber account cooldown group.',
          rangeError: 'Cyber account cooldown values must be whole seconds between 60 and 604800.',
          orderError: 'The repeated cooldown cannot be shorter than the first cooldown.',
        },
        affiliate: {
      ...
      gatewayForwarding: {
        ...
        codexTicketHarvestProxyDesc:
          "Shared by accounts with STATE enabled; normal requests keep each account's configured business egress. Paste a full HTTP / SOCKS5h proxy URL including credentials. A username {'{'}sid{'}'} placeholder or 1024proxy SID is replaced on every attempt. Changing the pool preserves valid tickets. Leave blank to keep the saved pool.",

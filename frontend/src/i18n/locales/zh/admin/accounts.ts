export default {
  ...
  "globalPoolHint": "这里只控制本账号是否使用票据。采集统一使用全局 IP 池，日常请求仍走本账号配置的业务出口。",
  "proxy": "采集代理",
  "proxyPlaceholder": "粘贴完整代理 URL（包含用户名和密码）",
  "proxyConfigured": "已配置：{address}。留空保存会保留现有连接。",
  "proxyHint": "仅用于获取 STATE。日常请求仍使用本账号上方已保存的业务出口；验证通过后才使用，约一小时有效并提前自动刷新。",
  "sidHint": "支持 HTTP / SOCKS5h 代理。动态代理用户名中的 {'{'}sid{'}'} 会在每次尝试时自动替换为新的会话标识。",
  "modelLabel": "目标模型",
  "modelHint": "只有所选模型使用票据，并在缺少有效票据时暂停请求；其他模型继续普通转发。更改模型后需重新获取票据。",
  "model": "目标模型：{model}",
  "ready": "可用，剩余 {time}",
  "savedTicket": "已保存票据",
  ...
  "fixedProxyUnsaved": "上方的业务出口配置尚未保存，请先保存账号并重新打开编辑窗口。",
  "businessEgressMissing": "尚无可验证的业务出口，请先保存账号代理或出口池配置。",
  ...
  "states": {
    ...
    "error": "获取失败"
      ...
      bulkEdit: {
        ...
        egressMixedFields: '静态 IP 出口与其他账号字段需分开保存。请先更新出口池，再编辑其余字段。',
        egressSavedOtherFieldsFailed: '静态 IP 出口已更新，但其他账号设置未全部保存：{reason}。请刷新后重试其他设置。',
        egressSavedConfirmOtherFields: '静态 IP 出口已经更新，其他账号设置尚未保存：{message}',
        egressSavedOtherFieldsCancelled: '静态 IP 出口已经更新，其他账号设置已取消保存。账号列表已刷新。',
      ...
      openai: {
        ...
        codexTurnTicketMissing: '等待 {length} STATE',
        codexTurnTicketReady: '剩余 {time}',
        codexTurnTicketPaused: '等待 {length} STATE，目标模型已暂停',
        codexTurnTicketMissingGeneric: '等待 STATE',
        codexTurnTicketPausedGeneric: '等待 STATE，目标模型已暂停',
        codexWarmupPolicy: '五小时窗口暖机',
        codexWarmupPolicyDesc: '账号导入后及上游自然 reset 到期时按策略发送最小 Codex 请求。',
        codexWarmupPolicyOff: '关闭',
        codexWarmupPolicyInitialOnce: '仅首次',
        codexWarmupPolicyContinuous: '持续',
        windowWarmup: {
          requeue: '重排暖机任务',
          unblock: '解除阻塞并重试',
          requeueSuccess: '暖机任务已重排',
          unblockSuccess: '暖机任务已解除阻塞',
          actionFailed: '暖机任务操作失败',
          nextRun: '下次执行',
          lastSuccess: '最近成功',
          observedReset: '上游重置',
          states: {
      ...
      anthropic: {
        ...
        metadataUserIdDesc:
          '默认关闭。开启后，metadata.user_id 使用 KKAI_ + 已验证的 NewAPI 用户 ID；直接调用本站时使用 KKAI_SUB2_ + 本站用户 ID。覆盖客户端传入的值，流式和非流式请求均生效。',
      ...
      egressPool: {
        title: '静态出口池',
        description: '为账号选择一个或多个固定出口 IP。系统只使用已选出口，不会自动直连回退。',
        direct: '本服务器直连',
        proxy: '代理节点',
        unnamedProxy: '代理节点 #{id}',
        primary: '认证主出口',
        primaryHint: 'OAuth 授权与首次认证固定使用此出口。',
        selectPrimary: '设为认证主出口',
        inherited: '继承母账号出口池',
        inheritedHint: 'Spark 影子账号由母账号统一管理出口与每出口并发，当前账号不可单独修改。',
        empty: '暂无可分配出口',
        catalogUnavailable: '出口路由目录暂不可用',
        routeCount: '{count} 个出口',
        noSelection: '请至少选择一个出口；不会隐式使用本服务器 IP。',
        primaryRequired: '请从已选出口中指定认证主出口。',
        perEgressConcurrency: '每出口并发',
        perEgressConcurrencyHint: '统一应用到所有已选出口。有效总容量由服务端按可用独立 IP 数量计算。',
        capacityFormula: '{eligible} 个可用 IP x {perEgress} = {capacity}',
        capacityDetail: '已配置 {configured}，可用 {eligible}，降级 {degraded}',
        degraded: '部分降级',
        verify: '验证出口',
        verifyFailed: '出口验证失败',
        reason: {
          route_not_found: '出口路由不存在',
          route_unavailable: '当前出口不可验证',
          probe_failed: '网络探测失败，请检查代理节点连通性',
          invalid_observation: '未获取到有效公网 IP',
          revision_conflict: '出口配置已变化，请刷新后重试',
          ...
          route_retired: '出口已退役',
          identity_unavailable: '公网 IP 身份不可用',
          direct_route_unavailable: '本服务器直连出口不可用',
          proxy_unavailable: '代理节点不可用',
          proxy_inactive: '代理节点已停用',
          proxy_expired: '代理节点已过期',
          route_kind_invalid: '出口类型无效',
          unknown: '出口验证失败（{code}）'
        },
        state: {
        ...
        bulk: {
          ...
          append: '追加',
          remove: '移除',
          replaceHint: '用所选出口完整替换账号当前出口池。',
          appendHint: '把所选出口追加到每个账号现有出口池。',
          removeHint: '从每个账号现有出口池移除所选出口。'
        }
      },
      loadFactor: '负载因子',

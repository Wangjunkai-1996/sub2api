    ...
    settings: {
      ...
      features: {
        ...
        cyberAccountCooldown: {
          title: 'OpenAI Cyber 账号冷却',
          description: '仅根据上游真实返回的 cyber_policy 冷却实际命中的账号，不发起额外审核请求。',
          enabled: '启用账号阶梯冷却',
          enabledHint: '仅对所选账号组中的 OpenAI OAuth Pro 账号生效；Plus、API Key 和其他平台账号不会进入冷却。',
          windowSeconds: '安静重置窗口（秒）',
          firstSeconds: '首次冷却（秒）',
          escalatedSeconds: '再次冷却（秒）',
          groupScope: '适用账号组',
          groupScopeHint: '默认账号组为 12。账号仍须同时满足 OpenAI、OAuth 和 plan_type=pro 才会命中。',
          searchGroups: '搜索分组名称、平台、类型或 ID',
          disabledGroup: '已停用',
          noGroups: '没有匹配的分组',
          groupsRequired: '请至少选择一个 Cyber 账号冷却组。',
          rangeError: 'Cyber 账号冷却时间必须是 60 到 604800 之间的整数秒。',
          orderError: '再次冷却时长不能短于首次冷却时长。',
        },
        affiliate: {
          title: '邀请返利',
      ...
      gatewayForwarding: {
        ...
        codexTicketHarvestProxyDesc:
          "供已开启 STATE 的账号统一采集票据，日常请求仍走各账号配置的业务出口。粘贴完整 HTTP / SOCKS5h 代理 URL（含用户名和密码）。用户名里的 {'{'}sid{'}'} 或 1024proxy 的 SID 每次尝试会自动更换。更换池不清除有效票据；留空保存保留现有配置。",

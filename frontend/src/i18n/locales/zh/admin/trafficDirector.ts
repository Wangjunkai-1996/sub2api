export default {
  trafficDirector: {
    ...
    history: {
      ...
      loadingDetails: '正在加载目标策略...',
      targetMode: '目标模式',
      targetChecksum: '目标 Checksum',
      targetUnassigned: '回滚后未分配账号 ID',
      targetSpec: '目标策略 Spec',
      none: '无',
    ...
    errors: {
      ...
      loadVersion: '加载目标策略版本失败。',
      preview: '策略 Preview 失败。',
      publish: '策略发布失败。',
      rollback: '策略回滚失败。',
      versionConflict: '服务端策略已变化，已刷新到最新版本，请审查后重试。',
      TRAFFIC_DIRECTOR_GROUP_NOT_FOUND: '所选 OpenAI 分组已不存在。',
      TRAFFIC_DIRECTOR_VERSION_NOT_FOUND: '所选策略版本已不存在。',
      TRAFFIC_DIRECTOR_VALIDATION_FAILED: '策略校验失败，请检查 Pool 权重、账号分配、备用链和确认项后重试。',
      TRAFFIC_DIRECTOR_VERSION_CONFLICT: '策略已从版本 {expected_version} 变化为 {actual_version}，已刷新到最新版本，请审查后重试。',
      TRAFFIC_DIRECTOR_IDEMPOTENCY_CONFLICT: '该操作标识已用于不同的策略内容，请重新发起操作。',
      TRAFFIC_DIRECTOR_POLICY_UNAVAILABLE: '当前流量策略暂时不可用，请等待控制面恢复后重试。',
      TRAFFIC_DIRECTOR_NO_AVAILABLE_POOL: '配置的 Pool 和备用链中没有可调度账号。'
    }
  }
}

...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	egressRepository := repository.NewEgressRepository(client, db)
	concurrencyService := service.ProvideConcurrencyService(concurrencyCache, accountRepository, egressRepository, configConfig)
	...
	egressService := service.NewEgressService(egressRepository, proxyExitInfoProber)
	privacyClientFactory := providePrivacyClientFactory()
	openAIOAuthService := service.ProvideOpenAIOAuthService(proxyRepository, openAIOAuthClient, settingService, egressService, privacyClientFactory)
	...
	openAIQuotaService := service.ProvideOpenAIQuotaService(accountRepository, proxyRepository, openAITokenProvider, privacyClientFactory, pluginManager, openAIReferralClient, openAIGatewayService, openAIOAuthService)
	...
	egressRouteHandler := admin.NewEgressRouteHandler(egressService, settingService)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	openAIWindowWarmupRepository := repository.NewOpenAIWindowWarmupRepository(db)
	openAIWindowOutboundAdapter := service.ProvideOpenAIWindowOutboundAdapter(accountRepository, openAITokenProvider, httpUpstream, tlsFingerprintProfileService, pluginManager, openAIGatewayService)
	openAIWindowWarmupOptions := service.ProvideOpenAIWindowWarmupOptions(settingService)
	openAICodexWindowProbe := service.ProvideOpenAICodexWindowProbe(openAIWindowOutboundAdapter, openAIWindowWarmupOptions)
	openAIWindowWarmupService := service.ProvideOpenAIWindowWarmupService(openAIWindowWarmupRepository, accountRepository, openAIWindowOutboundAdapter, openAICodexWindowProbe, auditLogService, openAIWindowWarmupOptions, concurrencyService, openAIQuotaService, rateLimitService)
	openCodeGoUsageService := service.ProvideOpenCodeGoUsageService(accountRepository, httpUpstream, settingService, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, egressRouteHandler, egressService, upstreamBillingProbeService, ollamaCloudUsageService, openAIWindowWarmupService, settingService, openAIGatewayService, openCodeGoUsageService)
	...
	egressIdentityVerificationWorker := service.ProvideEgressIdentityVerificationWorker(egressService)
	...
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, opsService, opsIngressRejectAggregator, apiKeyService, authCacheInvalidationWorker, egressIdentityVerificationWorker, schedulerSnapshotService, tokenRefreshService, accountExpiryService, cnProviderBalanceCheckService, openAICodexVersionSyncService, claudeCodeVersionSyncService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, batchImageCleanupService, batchImageWorkerRuntime, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, channelMonitorV2Aggregator, userPlatformQuotaUsageFlusher, upstreamBillingProbeService, ollamaCloudUsageService, openCodeGoUsageService, auditLogService, openAIQuotaAutoResetService, openAIWindowWarmupService, promptService, pluginManager)
	application := &Application{
		Server:             httpServer,
		PromptAudit:        promptService,
		PluginManager:      pluginManager,
		OpenAIWindowWarmup: openAIWindowWarmupService,
		Cleanup:            v,
	}

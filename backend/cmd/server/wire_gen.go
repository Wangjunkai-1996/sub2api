...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	openAIOAuthService := service.ProvideOpenAIOAuthService(proxyRepository, openAIOAuthClient, settingService, privacyClientFactory)
	...
	trafficDirectorRepository := repository.NewTrafficDirectorRepository(db)
	trafficDirectorPolicyStore := repository.ProvideTrafficDirectorPolicyStore(trafficDirectorRepository)
	trafficDirectorPolicyRedisCache := repository.ProvideTrafficDirectorPolicyRedisCache(redisClient)
	trafficDirectorPolicyCache := service.ProvideTrafficDirectorPolicyCache(trafficDirectorPolicyStore, trafficDirectorPolicyRedisCache)
	openAITrafficDirectorResolver := service.ProvideOpenAITrafficDirectorResolver(trafficDirectorRepository, trafficDirectorPolicyCache)
	trafficDirectorHealthStore := repository.ProvideTrafficDirectorHealthStore(redisClient)
	trafficDirectorHealthService := service.ProvideTrafficDirectorHealthService(trafficDirectorHealthStore)
	openAITrafficDirectorHealthResolver := service.ProvideOpenAITrafficDirectorHealthResolver(trafficDirectorHealthService)
	openAIGatewayService := service.ProvideConfiguredOpenAIGatewayService(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, serviceUserPlatformQuotaRepository, openAITrafficDirectorResolver, openAITrafficDirectorHealthResolver)
	...
	openAIQuotaService := service.ProvideOpenAIQuotaService(accountRepository, proxyRepository, openAITokenProvider, privacyClientFactory, pluginManager, openAIGatewayService)

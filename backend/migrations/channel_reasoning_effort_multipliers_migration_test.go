...
func TestChannelReasoningEffortMultipliersMigration(t *testing.T) {
	content, err := FS.ReadFile("246_channel_reasoning_effort_multipliers.sql")
	...
	require.Contains(t, sql, "entry || jsonb_build_object('reasoning_effort_multipliers'")
	require.NotContains(t, sql, "entry - 'max_reasoning_effort_multiplier'")
	require.Contains(t, sql, "Keep the legacy key during the rollback window")

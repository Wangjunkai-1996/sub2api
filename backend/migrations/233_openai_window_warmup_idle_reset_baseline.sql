-- Preserve the first idle five-hour reset as the durable comparison baseline.
-- /wham/usage may project an unused 0% window as observation-time + 5h. If an
-- account refresh also advances the job baseline, the worker cannot prove that
-- the projection is rolling and incorrectly arms the job for another five
-- hours. Active usage still replaces the baseline with its authoritative reset.
...
-- Repair untouched jobs that the previous trigger re-armed after a later 0%
-- usage refresh. Keep observed_reset_at unchanged so the worker can compare it
-- with a fresh passive observation before authorizing a send.

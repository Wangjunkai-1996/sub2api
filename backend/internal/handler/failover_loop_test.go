...
func TestSameAccountRetryAllowedRequiresCountAndDeadlineBudget(t *testing.T) {
	...
	require.False(t, sameAccountRetryAllowed(err, 0, 100), "do not wait past the deadline before replaying")
}
...
func TestHandleFailoverError_CacheBilling(t *testing.T) {
	...
	t.Run("OAuth deadline cannot extend an exhausted retry count", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := NewFailoverState(3, true)

...
func TestOpenAIWebSocketPreservesSubsequentTurnAudit(t *testing.T) {
	source := stripGoComments(goFunctionSource(t, "openai_gateway_handler.go", "ResponsesWebSocket"))
	require.NotContains(t, source, "checkSecurityAuditForSelectedOpenAIProAccount(")
	require.Contains(t, source, "if turn == 1 {")
	require.Contains(t, source, `checkSecurityAuditStage(c, reqLog, apiKey, subject, service.ContentModerationProtocolOpenAIResponses, model, payload, "subsequent_turn")`)
	require.Contains(t, source, "writeSecurityAuditWSError(ctx, wsConn, decision)")
}

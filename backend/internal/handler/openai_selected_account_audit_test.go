...
func TestOpenAIGatewayHandlerResponses_UnknownFieldWithProOnlyPoolDispatchesAfterAudit(t *testing.T) {
	upstream := &selectedAccountAuditResponsesUpstream{}
	engine := &turnCountingEngine{mode: securityaudit.ModeBlocking}
	handler, cache := newSelectedAccountAuditHandler(t, upstream, engine)
	c, recorder := selectedAccountAuditHTTPContext(
		t,
		"/v1/responses",
		`{"model":"gpt-5.1","stream":true,"input":"unknown-field-pro-canary","text":{"future":{"description":"hidden"}}}`,
	)

	handler.Responses(c)

	require.Equal(t, http.StatusOK, recorder.Code, recorder.Body.String())
	require.Equal(t, []int64{1}, upstream.calls(), "unknown outer fields must not exclude OAuth Pro before selection")
	require.Equal(t, int64(1), engine.evaluates.Load(), "the selected Pro account must still run the lightweight audit")
	require.Equal(t, int32(1), atomic.LoadInt32(&cache.releaseAccountCalled))
	require.Equal(t, int32(1), atomic.LoadInt32(&cache.releaseUserCalled))
}

...
func TestAdvancedSchedulerRoutingTierDoesNotConsumeFallbackProbeBudget(t *testing.T) {
	...
	for id := int64(1); id <= openAIAccountSelectionProbeLimit; id++ {
		account := &Account{
			...
			Concurrency: 1, GroupIDs: []int64{12}, Credentials: map[string]any{"plan_type": "plus"},
		}
		selectionOrder = append(selectionOrder, openAIAccountCandidateScore{
			account: account, loadInfo: &AccountLoadInfo{AccountID: id}, loadKnown: false,
		})
	...
	apiKey := &Account{
		ID: openAIAccountSelectionProbeLimit + 1, Platform: PlatformOpenAI, Type: AccountTypeAPIKey,
		Status: StatusActive, Schedulable: true, Concurrency: 1,
	}
	selectionOrder = append(selectionOrder, openAIAccountCandidateScore{
		account: apiKey, loadInfo: &AccountLoadInfo{AccountID: apiKey.ID}, loadKnown: false,
	})
	...
	selection, _, err := scheduler.tryAcquireOpenAISelectionOrder(
		context.Background(),
		OpenAIAccountScheduleRequest{Platform: PlatformOpenAI, Routing: auditedOAuthRoutingOptions()},
		selectionOrder,
	)
...
func advancedSchedulerRoutingTierProbeFixture() (
	...
	*Account,
	...
	selectionOrder := make([]openAIAccountCandidateScore, 0, openAIAccountSelectionProbeLimit+1)
	snapshotAccounts := make(map[int64]*Account, openAIAccountSelectionProbeLimit+1)
	latestAccounts := make(map[int64]*Account, openAIAccountSelectionProbeLimit+1)
	for id := int64(1); id <= openAIAccountSelectionProbeLimit; id++ {
		stalePro := &Account{
			ID: id, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true,
			Concurrency: 1, GroupIDs: []int64{12}, Credentials: map[string]any{"plan_type": "pro"},
		}
		freshPlus := *stalePro
		freshPlus.Credentials = map[string]any{"plan_type": "plus"}
		snapshotAccounts[id] = stalePro
		latestAccounts[id] = &freshPlus
		selectionOrder = append(selectionOrder, openAIAccountCandidateScore{
			account: stalePro, loadInfo: &AccountLoadInfo{AccountID: id}, loadKnown: false,
		})
	}
	apiKey := &Account{
		ID: openAIAccountSelectionProbeLimit + 1, Platform: PlatformOpenAI, Type: AccountTypeAPIKey,
	...
	snapshotAccounts[apiKey.ID] = apiKey
	latestAccounts[apiKey.ID] = apiKey
	selectionOrder = append(selectionOrder, openAIAccountCandidateScore{
		account: apiKey, loadInfo: &AccountLoadInfo{AccountID: apiKey.ID}, loadKnown: false,
	})
	return selectionOrder, snapshotAccounts, latestAccounts, apiKey
}

func TestAdvancedSchedulerReservesAcquireAndRecheckProbeForFallbackRoutingTier(t *testing.T) {
	selectionOrder, snapshotAccounts, latestAccounts, apiKey := advancedSchedulerRoutingTierProbeFixture()
	...
	require.Equal(t, openAIAccountSelectionProbeLimit-1, repo.calls())
	require.Equal(t, openAIAccountSelectionProbeLimit-1, budget.acquires)
	require.Equal(t, openAIAccountSelectionProbeLimit-1, budget.rechecks)
	selection.ReleaseFunc()
}

func TestAdvancedSchedulerReservesFallbackDBRecheckForLaterRoutingTier(t *testing.T) {
	selectionOrder, snapshotAccounts, latestAccounts, apiKey := advancedSchedulerRoutingTierProbeFixture()
	repo := &upstreamCostCountingAccountRepo{accounts: latestAccounts}
	scheduler := &defaultOpenAIAccountScheduler{service: &OpenAIGatewayService{
		accountRepo:       repo,
	...
	budget := newOpenAISelectionProbeBudget()
	budget.enableLimit()
	req := OpenAIAccountScheduleRequest{Platform: PlatformOpenAI, Routing: auditedOAuthRoutingOptions()}

	selection, _, _, _, err := scheduler.finishLoadBalanceSelectionFallback(
		context.Background(),
		req,
		openAIAccountLoadSelectionAttempt{selectionOrder: selectionOrder, candidateCount: len(selectionOrder)},
		budget,
	...
	require.Equal(t, apiKey.ID, selection.Account.ID)
	require.Equal(t, apiKey.ID, selection.WaitPlan.AccountID)
	require.Equal(t, openAIAccountSelectionProbeLimit-1, repo.calls())
	require.Equal(t, openAIAccountSelectionProbeLimit-1, budget.rechecks)
}

...
const codexAccountTicketConfigKey = "codex_turn_ticket:v2:config"
...
func codexAccountTicketEligible(account *Account) bool {
	return isOpenAICodexTicketAccount(account) && account.Status == StatusActive && codexTicketFixedProxyFingerprint(account) != ""
}
...
func (s *OpenAIGatewayService) codexTicketLiveAccount(ctx context.Context, account *Account) (*Account, error) {
	...
	return PreserveSelectedAccountEgress(live, account)
}
...
func (s *OpenAIGatewayService) GetCodexAccountTicketStatus(ctx context.Context, id int64) (*CodexAccountTicketStatus, error) {
	...
	status := &CodexAccountTicketStatus{TicketPlan: ac.TicketPlan, TargetLength: codexTicketTargetLength(ac.TicketPlan), Enabled: ac.Enabled, GlobalEnabled: s.openAICodexTicketEnabledContext(ctx), Model: ac.Model, ProxyConfigured: poolConfigured, FixedProxyConfigured: s.codexTicketBusinessReady(ctx, account), State: "waiting"}
	...
	if !s.codexTicketBusinessReady(ctx, account) {
		status.State = "error"
...
func (s *OpenAIGatewayService) ConfigureCodexAccountTicket(ctx context.Context, id int64, input CodexAccountTicketUpdate) (*CodexAccountTicketStatus, error) {
	...
	pool := s.openAICodexTicketHarvestProxyURLContext(ctx)
	if next.Enabled && (pool == "" || ValidateOpenAICodexTicketHarvestProxyURL(pool) != nil || !s.codexTicketBusinessReady(ctx, account)) {
		s.openaiCodexAccountMu.Unlock()
	...
	if changed {
		...
		for key := range account.Extra {
			if key != codexAccountTicketConfigKey && key != codexTicketWatchdogExtraKey && key != openAICodexTicketExtraKeyPrefix+"lease" && strings.HasPrefix(key, openAICodexTicketExtraKeyPrefix) {
				updates[key] = nil
			}
		}
		if err := s.updateCodexTicketExtra(ctx, account, updates); err != nil {
			s.openaiCodexAccountMu.Unlock()
		...
		if err := s.updateCodexTicketExtra(ctx, account, map[string]any{codexAccountTicketConfigKey: next}); err != nil {
			s.openaiCodexAccountMu.Unlock()
			return nil, apperrors.New(500, "CODEX_TICKET_SAVE_FAILED", "Could not save account ticket settings")
		}
	}
...
func (s *OpenAIGatewayService) startCodexAccountTicketJob(ctx context.Context, id int64, manual bool) *codexAccountTicketJob {
	...
	account, err := s.codexTicketAccountByID(ctx, id)
	if err != nil || !s.codexTicketBusinessReady(ctx, account) {
		return nil
	}
	ac := codexAccountTicketConfigOf(account)
	pool := s.openAICodexTicketHarvestProxyURLContext(ctx)
...
func (s *OpenAIGatewayService) runCodexAccountTicketJob(ctx context.Context, id int64, job *codexAccountTicketJob) {
	...
	initial, err := s.codexTicketAccountByID(ctx, id)
	if err != nil {
		return
	}
	// Refresh credentials before fencing the collector's identity snapshot.
	if token, _, err := s.GetAccessToken(ctx, initial); err != nil || token == "" {
	...
	if !ok {
		lastError = "Ticket persistence is unavailable"
		return
	}
	timeout := time.Duration(s.openAICodexTicketConfig().HarvestAttemptTimeoutSeconds) * time.Second
	leaseTTL := 2*timeout + 30*time.Second
	lease, err := store.AcquireCodexTicketLease(ctx, initial, codexAccountTicketConfigOf(initial).Model, CodexTicketExtraSnapshot(initial, codexAccountTicketConfigKey), leaseTTL)
	if err != nil || lease == nil {
		lastError = "Ticket acquisition is already running or unavailable"
		return
	}
	defer func() {
		releaseCtx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
		_, _ = store.ReleaseCodexTicketLease(releaseCtx, lease)
	}()
	selected, release, err := s.acquireCodexTicketBusinessAccount(ctx, initial)
	if err != nil {
		lastError = "Business egress is unavailable"
		return
	}
	defer release()
	for attempt := 1; attempt <= codexTicketMaxAttempts; attempt++ {
		...
		account, err := s.codexTicketLiveAccount(ctx, selected)
		if err != nil {
			return
		}
		ac := codexAccountTicketConfigOf(account)
		...
		if renewed, err := store.RenewCodexTicketLease(ctx, account, lease, leaseTTL); err != nil || !renewed {
			lastError = "Ticket acquisition lease expired"
			return
		}
		...
		if err != nil || status != 200 || !validCodexTicketState(state) {
			...
			replayState, status, err := s.fireCodexAccountTicketProbe(ctx, account, token, ac.Model, codexTicketBusinessProxyURL(account), state, timeout)
			if reason := codexTicketProbeRejection(status); reason != "" {
				lastError = reason
				return
			}
			if err == nil && status == 200 && (len(replayState) != 312 || !validCodexTicketState(replayState)) {
				...
				s.openaiCodexAccountMu.Lock()
				live, readErr := s.codexTicketLiveAccount(ctx, account)
				if readErr == nil && ctx.Err() == nil && s.openaiCodexAccountJobs[id] == job && s.openAICodexTicketEnabledContext(ctx) && s.openAICodexTicketHarvestProxyURLContext(ctx) == job.harvestProxyURL && codexAccountTicketEligible(live) && codexAccountTicketConfigOf(live).Enabled && codexAccountTicketConfigOf(live).Revision == job.revision && codexTicketFixedProxyFingerprint(live) == job.fixedFingerprint {
					now := time.Now()
					ticket := &openAICodexTicket{AccountID: id, Model: ac.Model, State: state, Length: len(state), CapturedAt: now, ExpiresAt: now.Add(time.Hour), Attempts: attempt, Verified: true, ConfigRevision: job.revision, FixedProxyFingerprint: job.fixedFingerprint, EgressBindingID: codexTicketBusinessBinding(live)}
					s.storeOpenAICodexTicketWithLease(ctx, live, ticket, lease)

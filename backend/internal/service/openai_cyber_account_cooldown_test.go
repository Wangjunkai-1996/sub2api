	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			repo.onSet = func() {
				...
				require.True(t, svc.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4"), "request fastpath must preserve the Cyber block before persistence")
...
func TestOpenAICyberAccountCooldownPendingGateBlocksSchedulingDuringDedup(t *testing.T) {
	...
	require.True(t, svc.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4"), "request fastpath must preserve the pending classification gate")
	close(release)
	result := <-resultCh
...
func TestOpenAICyberAccountCooldownPersistenceFailureKeepsRuntimeBlock(t *testing.T) {
	...
	require.True(t, svc.isOpenAIAccountRuntimeBlocked(account))
	require.True(t, svc.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4"), "failed persistence must not clear the Cyber runtime deadline")
}
...
func TestOpenAICyberAccountCooldownPersistenceFailureBlocksAnotherServiceInstance(t *testing.T) {
	...
	require.True(t, reader.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4"), "request fastpath must preserve the Redis-derived deadline")
...
func TestOpenAICyberAccountCooldownRuntimeDeadlineCleanup(t *testing.T) {
	...
	require.False(t, svc.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4"), "expired Cyber state must not retain an ordinary stale cooldown")

...
func (r *openAICyberCooldownSettingRepo) GetValue(_ context.Context, key string) (string, error) {
...
func openAICyberCooldownEligibleAccount() *Account {
	return &Account{
		ID:          10616,
		Platform:    PlatformOpenAI,
		Type:        AccountTypeOAuth,
		Credentials: map[string]any{"plan_type": "pro"},
		GroupIDs:    []int64{openAICyberAccountCooldownProGroupID},
	}
}

func TestOpenAICyberAccountCooldownEligibility(t *testing.T) {
	tests := []struct {
		name    string
		account *Account
		want    bool
		...
		{name: "team oauth in pro pool", account: &Account{
			ID: 3, Platform: PlatformOpenAI, Type: AccountTypeOAuth,
			Credentials: map[string]any{"plan_type": "team"}, GroupIDs: []int64{openAICyberAccountCooldownProGroupID},
		}},
		{name: "free oauth in pro pool", account: &Account{
			ID: 4, Platform: PlatformOpenAI, Type: AccountTypeOAuth,
			Credentials: map[string]any{"plan_type": "free"}, GroupIDs: []int64{openAICyberAccountCooldownProGroupID},
		}},
		{name: "pro oauth outside pro pool", account: &Account{
			ID: 5, Platform: PlatformOpenAI, Type: AccountTypeOAuth,
			Credentials: map[string]any{"plan_type": "pro"}, GroupIDs: []int64{13},
		}},
		{name: "pro api key in pro pool", account: &Account{
			ID: 6, Platform: PlatformOpenAI, Type: AccountTypeAPIKey,
			Credentials: map[string]any{"plan_type": "pro"}, GroupIDs: []int64{openAICyberAccountCooldownProGroupID},
		}},
		{name: "pro setup token in pro pool", account: &Account{
			ID: 7, Platform: PlatformOpenAI, Type: AccountTypeSetupToken,
			Credentials: map[string]any{"plan_type": "pro"}, GroupIDs: []int64{openAICyberAccountCooldownProGroupID},
		}},
		{name: "non openai", account: &Account{
			ID: 8, Platform: PlatformGrok, Type: AccountTypeOAuth,
			Credentials: map[string]any{"plan_type": "pro"}, GroupIDs: []int64{openAICyberAccountCooldownProGroupID},
		}},
		{name: "missing group metadata", account: &Account{
...
func TestOpenAICyberAccountCooldownIneligibleAccountsSkipAllState(t *testing.T) {
	tests := []struct {
		...
		{name: "plus", account: &Account{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Credentials: map[string]any{"plan_type": "plus"}, GroupIDs: []int64{12}}},
		{name: "other pool", account: &Account{ID: 2, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Credentials: map[string]any{"plan_type": "pro"}, GroupIDs: []int64{13}}},
		{name: "api key", account: &Account{ID: 3, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Credentials: map[string]any{"plan_type": "pro"}, GroupIDs: []int64{12}}},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			result := svc.ApplyCyberPolicyAccountCooldown(context.Background(), tc.account, OpenAICyberAccountCooldownEvent{RequestID: "req-ineligible"})
			err := svc.RecheckOpenAICyberAccountCooldown(context.Background(), tc.account)

			require.False(t, result.Applied)
			require.NoError(t, err)
			require.Zero(t, cache.calls, "ineligible account must not write a strike")
			require.Zero(t, cache.deadlineCalls, "ineligible account must not read a legacy deadline")
			require.Zero(t, repo.calls, "ineligible account must not be persisted as unschedulable")
			require.False(t, svc.isOpenAICyberCooldownClassificationPending(tc.account.ID))
			require.False(t, svc.isOpenAIAccountRuntimeBlocked(tc.account))
		})
	}
}
...
func TestOpenAICyberAccountCooldownFirstAndEscalatedTiers(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			repo := &openAICyberCooldownAccountRepo{}
			svc := newOpenAICyberCooldownService(true, cache, repo)
			account := openAICyberCooldownEligibleAccount()
			repo.onSet = func() {
				require.True(t, svc.isOpenAIAccountRuntimeBlocked(account), "runtime block must precede durable persistence")
				require.True(t, svc.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4", false), "request fastpath must preserve the Cyber block before persistence")
...
func TestOpenAICyberAccountCooldownPendingGateBlocksSchedulingDuringDedup(t *testing.T) {
	...
	require.True(t, svc.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4", false), "request fastpath must preserve the pending classification gate")
	close(release)
	result := <-resultCh
...
func TestOpenAICyberAccountCooldownUsesShortRedisDeadline(t *testing.T) {
	...
	result := svc.ApplyCyberPolicyAccountCooldown(context.Background(), openAICyberCooldownEligibleAccount(), OpenAICyberAccountCooldownEvent{RequestID: "req-deadline"})
...
func TestOpenAICyberAccountCooldownDisabledOrNonOpenAISkips(t *testing.T) {
	...
	require.False(t, disabled.ApplyCyberPolicyAccountCooldown(context.Background(), openAICyberCooldownEligibleAccount(), OpenAICyberAccountCooldownEvent{}).Applied)
...
func TestOpenAICyberAccountCooldownPersistenceFailureKeepsRuntimeBlock(t *testing.T) {
	...
	require.True(t, svc.isOpenAIAccountRuntimeBlocked(account))
	require.True(t, svc.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4", false), "failed persistence must not clear the Cyber runtime deadline")
}
...
func TestOpenAICyberAccountCooldownPersistenceFailureBlocksAnotherServiceInstance(t *testing.T) {
	...
	require.True(t, reader.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4", false), "request fastpath must preserve the Redis-derived deadline")
	require.Equal(t, 1, cache.deadlineCalls)
}

func TestOpenAICyberAccountCooldownRuntimeDeadlineCleanup(t *testing.T) {
	svc := &OpenAIGatewayService{}
	account := openAICyberCooldownEligibleAccount()
	svc.BlockAccountScheduling(account, time.Now().Add(time.Hour), openAICyberAccountCooldownReason+":redis_fallback")
	require.True(t, svc.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4", false))
	svc.ClearAccountSchedulingBlock(account.ID)
	require.False(t, svc.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4", false))

	svc.BlockAccountScheduling(account, time.Now().Add(time.Hour), "transport_error")
	svc.openaiLocalCooldownRuntimeUntil.Store(account.ID, time.Now().Add(-time.Second))
	require.False(t, svc.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4", false), "expired Cyber state must not retain an ordinary stale cooldown")
	_, exists := svc.openaiLocalCooldownRuntimeUntil.Load(account.ID)

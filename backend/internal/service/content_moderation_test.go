...
func TestExtractStrictCurrentUserText(t *testing.T) {
	longText := strings.Repeat("界", 12_000) + strings.Repeat("尾", 17)
	...
	tests := []struct {
		...
		{
			name:     "chat audits final tool message without rewinding history",
			protocol: ContentModerationProtocolOpenAIChat,
			body: []byte(`{"messages":[
			...
			want:      "later tool output",
			forbidden: "latest user",
		},
		{
		...
		{
			name:     "text beyond former boundary remains complete",
			protocol: ContentModerationProtocolOpenAIResponses,
			body:     longBody,
			want:     longText,
		},
...
func TestContentModerationCheck_StrictSendsFullCurrentUserTextBeyondFormerLimit(t *testing.T) {
	groupID := int64(12)
	want := strings.Repeat("界", 12_000) + strings.Repeat("尾", 17)
	body, err := json.Marshal(map[string]any{
		"input": []any{map[string]any{
			"type": "message", "role": "user",
			"content": []any{
	...
	cfg.APIKeys = []string{"sk-test"}
	cfg.MaxRPM = 100
	cfg.BlockedKeywords = []string{"blocked-after-former-limit"}
	rawCfg, err := json.Marshal(cfg)
	require.NoError(t, err)
	svc := &ContentModerationService{
		settingRepo: &contentModerationTestSettingRepo{values: map[string]string{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for attempt := 1; attempt <= 2; attempt++ {
				decision, err := svc.Check(context.Background(), ContentModerationCheckInput{
					Strict: true, APIKeyID: 7, GroupID: &groupID, Endpoint: "/v1/moderations-test", Model: "gpt-test",
...
func TestContentModerationCheck_StrictMalformedImageRequestIsRejectedWithoutModerationCall(t *testing.T) {
	...
	require.EqualError(t, err, "strict content moderation input is incomplete")
	require.Nil(t, decision)
	require.Zero(t, svc.preBlockChecked.Load())
	require.Zero(t, svc.preBlockAllowed.Load())
	mu.Lock()
	defer mu.Unlock()
	require.Zero(t, requestCount)
}
...
func TestContentModerationCheck_Strict59ToolScreenshotsAreNotModeratedUnderConcurrency(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if isText {
			textRequests++
			if got := len([]rune(text)); got != maxModerationInputRunes && handlerErr == nil {
				handlerErr = fmt.Errorf("moderation text rune count = %d, want %d", got, maxModerationInputRunes)
			}
	...
	for index := 0; index < workers; index++ {
		...
		go func() {
			defer wg.Done()
			decision, checkErr := svc.Check(context.Background(), ContentModerationCheckInput{
				Strict: true, APIKeyID: 7, GroupID: &groupID, Endpoint: "/v1/responses", Model: "gpt-test",
				Protocol: ContentModerationProtocolOpenAIResponses, Body: body,
			})
			if checkErr != nil {
				errCh <- checkErr

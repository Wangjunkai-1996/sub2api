...
func TestCodexTicketWSHTTPBridgeRechecksEveryTurn(t *testing.T) {
	for _, change := range []string{"refresh", "expire", "account_off", "global_off"} {
		t.Run(change, func(t *testing.T) {
			...
			svc.settingService.openAICodexTicketEnabledCache.Store(&cachedOpenAICodexTicketSettings{enabled: true, expiresAt: time.Now().Add(time.Hour).UnixNano()})
			...
			case "global_off":
				svc.settingService.openAICodexTicketEnabledCache.Store(&cachedOpenAICodexTicketSettings{enabled: false, expiresAt: time.Now().Add(time.Hour).UnixNano()})
			}
			writeTurn()
			if change == "expire" {
				select {

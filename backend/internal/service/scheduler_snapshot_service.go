...
// GetAccountMetadataByIDs resolves scheduler metadata in bulk. The returned map
// contains every distinct requested ID; a nil value means the ID could not be
// resolved and lets security-sensitive callers fail closed without issuing a
// per-ID fallback. Cache misses are hydrated by at most one GetByIDs call.
//
// Accounts returned by this method are suitable for scheduling and policy
// classification only. Cache hits intentionally contain filtered scheduler
// metadata rather than complete forwarding credentials.
func (s *SchedulerSnapshotService) GetAccountMetadataByIDs(ctx context.Context, accountIDs []int64) (map[int64]*Account, error) {
	accountsByID := make(map[int64]*Account, len(accountIDs))
	pendingIDs := make([]int64, 0, len(accountIDs))
	...
	if s != nil && s.cache != nil {
		if reader, ok := s.cache.(SchedulerAccountMetadataBatchReader); ok {
			cached, err := reader.GetAccountMetadataByIDs(ctx, pendingIDs)
			if ctxErr := ctx.Err(); ctxErr != nil {
				return accountsByID, ctxErr
			}
			if err != nil {
				logger.LegacyPrintf("service.scheduler_snapshot", "[Scheduler] account metadata cache batch read failed: count=%d err=%v", len(pendingIDs), err)
			} else {
				misses := pendingIDs[:0]
				for _, accountID := range pendingIDs {
					account := cached[accountID]
	...
	if s == nil || s.accountRepo == nil {
		return accountsByID, ErrSchedulerCacheNotReady
	}
	if err := s.guardFallback(ctx); err != nil {
		return accountsByID, err
	}
	...
	fallbackCtx, cancel := s.withFallbackTimeout(ctx)
	defer cancel()
	loaded, err := s.accountRepo.GetByIDs(fallbackCtx, pendingIDs)
	if err != nil {
		return accountsByID, err
	}
	if ctxErr := ctx.Err(); ctxErr != nil {
	...
	for _, accountID := range pendingIDs {
		pendingSet[accountID] = struct{}{}
	}
	for _, account := range loaded {
		if account == nil {
			continue
		}
		if _, requested := pendingSet[account.ID]; !requested {
			continue
		}

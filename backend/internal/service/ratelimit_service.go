...
func (s *RateLimitService) HandleOpenAIWindowWarmupAuthFailure(ctx context.Context, failure OpenAIWindowWarmupAuthFailure) error {
	...
	case OpenAIWindowWarmupAuthRefreshTransient:
		if account.IsOpenAIAgentIdentity() {
			...
			s.notifyAccountSchedulingBlocked(account, until, "openai_window_warmup:openai_agent_identity_recovery")
	...
	case OpenAIWindowWarmupAuthForbidden:
		...
		s.handleOpenAI403WithPermanentHandler(ctx, account, "", []byte(`{"error":{"message":"access forbidden"}}`), "openai_window_warmup:openai_403_temp", func(message string) {
...
func (s *RateLimitService) persistOpenAIWarmupTransientAuthState(ctx context.Context, account *Account) error {
	...
	s.notifyAccountSchedulingBlocked(account, until, "openai_window_warmup:oauth_401")
...
func (s *RateLimitService) setOpenAIWindowAuthError(ctx context.Context, account *Account, expectedCredentials map[string]any, message string) (bool, error) {
	...
	s.notifyAccountSchedulingBlocked(account, time.Time{}, "openai_window_warmup:auth_error")
	if s.tokenCacheInvalidator != nil {
...
func (s *RateLimitService) handleOpenAI403(ctx context.Context, account *Account, upstreamMsg string, responseBody []byte) (shouldDisable bool) {
	return s.handleOpenAI403WithPermanentHandler(ctx, account, upstreamMsg, responseBody, "openai_403_temp", func(message string) {
...
func (s *RateLimitService) handleOpenAI403WithPermanentHandler(
	...
	s.notifyAccountSchedulingBlocked(account, until, runtimeBlockReason)
...
func (s *RateLimitService) handle429(ctx context.Context, account *Account, headers http.Header, responseBody []byte) {
	// Shared recovery already owns transient OAuth 429 admission. Do not stack
	// the generic persistent fallback on top of its short recovery rounds.
	if usesOpenAI429RecoveryAdmission(ctx, account, headers, responseBody) {
		return
	}

...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (result *ForwardResult, err error) {
	...
	// 调试日志：记录即将转发的账号信息
	logger.LegacyPrintf("service.gateway", "[Forward] Using account: ID=%d Name=%s Platform=%s Type=%s TLSFingerprint=%v ProxyConfigured=%t EgressMode=%s",
		account.ID, account.Name, account.Platform, account.Type, tlsProfile, proxyURL != "", account.EgressMode)

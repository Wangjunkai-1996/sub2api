...
// legacyV23APIKeyAuthCacheEntry models the fields decoded by the production
...
type legacyV23APIKeyAuthCacheEntry struct {
	Snapshot *struct {
...
func TestAPIKeyAuthSnapshotBridgeRoundTrip(t *testing.T) {
	...
	var legacy legacyV23APIKeyAuthCacheEntry
	require.NoError(t, json.Unmarshal(payload, &legacy))
	require.NotNil(t, legacy.Snapshot)
	require.Equal(t, 23, legacy.Snapshot.Version)
...
func TestAPIKeyAuthSnapshotVersionCompatibility(t *testing.T) {
	tests := []struct {
		...
		{name: "native v24", wireVersion: 24, wantUsed: true},
		{name: "v24 bridge", wireVersion: 23, completenessVersion: 24, wantUsed: true},
		{name: "legacy v22", wireVersion: 22, wantUsed: false},
		{name: "legacy v23 without marker", wireVersion: 23, wantUsed: false},
		{name: "v23 with unknown marker", wireVersion: 23, completenessVersion: 25, wantUsed: false},
		{name: "unknown wire version", wireVersion: 25, completenessVersion: 24, wantUsed: false},
	}

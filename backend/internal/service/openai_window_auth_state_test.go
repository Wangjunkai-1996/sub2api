...
func TestRateLimitServiceOpenAIWindowPermanent401UsesCredentialsCAS(t *testing.T) {
	...
	require.Equal(t, "openai_window_warmup:auth_error", blocker.reason)
}
...
func TestRateLimitServiceOpenAIWindowTransient401PropagatesCooldownPersistenceFailure(t *testing.T) {
	...
	require.ErrorContains(t, err, "database unavailable")
	require.Equal(t, 1, repo.tempCalls)
	require.True(t, blocker.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4", false), "failed warmup persistence must preserve the local scheduler cooldown")
}
...
func TestRateLimitServiceOpenAIWindowAgentIdentityTransientRecoveryDoesNotPermanentlyDisable(t *testing.T) {
	...
	require.True(t, blocker.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4", false), "warmup recovery must keep the cooldown while the scheduling snapshot is stale")
}
...
func TestRateLimitServiceOpenAIWindow403ReusesExistingPolicyWithoutRawBody(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			require.Equal(t, test.wantTemp+test.wantCAS > 0, blocker.isOpenAIAccountRequestRuntimeBlocked(account, "gpt-5.4", false))

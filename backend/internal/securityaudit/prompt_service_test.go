	...
	tests := []struct {
	...
	}{
		{
			...
			body: `{"messages":[
				...
				{"role":"user","content":[{"type":"image_url","image_url":{"url":"opaque-image"}}]}
			]}`,
		},
		{
...
func TestPromptServiceStrictBlockingUsesOneCompletePrimaryScan(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, DecisionAllow, decision.Kind)
	require.Equal(t, []scanCall{{endpoint: "primary", text: latest}}, calls)
	require.Equal(t, len([]rune(latest)), len([]rune(calls[0].text)))
}
...
func TestPromptServiceStrictChatToolOutputIsScanned(t *testing.T) {
	var scanned string
	evaluator := newGuardEvaluator(PromptScannerFunc(func(_ context.Context, _ ActiveEndpoint, text string, _ []string) (*NormalizedResult, error) {
		scanned = text
		return &NormalizedResult{Action: ActionAllow}, nil
	...
	body := []byte(`{"messages":[{"role":"user","content":"historical user"},{"role":"tool","tool_call_id":"call_1","content":"tool output"}]}`)
	document := auditinput.ParseForTextAudit(auditinput.ProtocolOpenAIChat, body)

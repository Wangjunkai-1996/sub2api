...
func insertJob(ctx context.Context, queryer sqlQueryer, snapshot PromptSnapshot, mode Mode, configVersion int64, status string, maxAttempts int) (*Job, error) {
	...
	row := queryer.QueryRowContext(ctx, `
		INSERT INTO prompt_audit_jobs (
			...
			prompt_length,message_count,priority_prefix_runes,stage,execution_mode,config_version,status,max_attempts,processed_at
		...
		snapshot.RedactedPreview, snapshot.PromptLength, snapshot.MessageCount, snapshot.PriorityPrefixRunes,
		normalizeStage(snapshot.Stage), string(mode), configVersion, status, maxAttempts)
	return scanJob(row)
}
...
func scanJob(row rowScanner) (*Job, error) {
	...
	err := row.Scan(
		...
		&job.Snapshot.RedactedPreview, &job.Snapshot.PromptLength, &job.Snapshot.MessageCount,
		&job.Snapshot.PriorityPrefixRunes, &job.Snapshot.Stage,
...
func jobColumns(alias string) string {
	return fmt.Sprintf(`%[1]s.id,%[1]s.request_id,%[1]s.user_id,%[1]s.username_snapshot,%[1]s.user_email_snapshot,
		...
		%[1]s.prompt_length,%[1]s.message_count,%[1]s.priority_prefix_runes,%[1]s.stage,%[1]s.execution_mode,%[1]s.config_version,%[1]s.status,
